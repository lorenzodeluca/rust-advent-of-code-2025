...
/// ```
fn solve(mut input: Vec<Vec<u64>>, connections_limit: i32) -> u64 {
     /*
        reasoning:
    ...
    //TODO: known ignored bug, if there are less than 3 circuit of length >1 the code doesnt work...
    return circuit_size[circuit_size.len()-1]*circuit_size[circuit_size.len()-2]*circuit_size[circuit_size.len()-3];
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    print!(
        "{}",
        solve(read_file_to_matrix(input_file.to_string())?,1000)
    );
